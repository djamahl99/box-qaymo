...
def run_on_dataset(
    ...
    tokenizer, model, image_processor, context_len = load_pretrained_model(
        args.base_path, args.model_base, model_name, args.load_8bit, args.load_4bit
    )

    if args.lora_path is not None:
        # Then load your fine-tuned LoRA weights
    ...
    for sample in tqdm(
        gt_dataset.samples,
        desc="Running LLaVA on samples",
        total=len(gt_dataset.samples),
    ):
...
def main():
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help="Path to load lora weights",
    )
    parser.add_argument(
    ...
    llava_args = argparse.Namespace(
        base_path="liuhaotian/llava-v1.5-7b",  # Specify the correct model path
    ...
    if not save_path.exists():
        pred_dataset = run_on_dataset(
            ...
            save_path,
            args.batch_size,
            draw_bbox=not args.dont_draw_bbox,
        )
    else:
    ...
    metric.print_latex_tables(metric_results, model_name)

    metric_latex_save_path = Path(metrics_save_path).with_suffix(".tex")
    metric.save_latex_tables(metric_results, model_name, str(metric_latex_save_path))
