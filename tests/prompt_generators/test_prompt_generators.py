...
class TestPromptGeneratorInterface(unittest.TestCase):
        ...
        for gen_class in generators:
            for method_name in required_methods:
                ...
                self.assertIsNotNone(
                    method, 
                    f"Method {method_name} not found in {gen_class.__name__}"
                )
                
                # Check if the method is not abstract
                if hasattr(method, '__isabstractmethod__') and method.__isabstractmethod__:
                    self.fail(f"Method {method_name} in {gen_class.__name__} is abstract")
                ...
                if base_method is not None:
                    ...
                    if class_method is inspect.getattr_static(BasePromptGenerator, method_name, None):
                        # Just a warning, not a failure, as default implementations are OK
                        print(f"Warning: {gen_class.__name__} uses default implementation of {method_name}")
                    ...
                    try:
                        ...
                        for name, obj in inspect.getmembers(module):
                            if (inspect.isclass(obj) and 
                                issubclass(obj, BasePromptGenerator) and 
                                obj != BasePromptGenerator):
    ...
    def test_generator_implementations_work(self):
        ...
        for gen_class in generators:
            ...
            self.assertTrue(isinstance(metric, BaseMetric),
            ...
            self.assertIsNotNone(answer_type, 
                                f"{gen_class.__name__}.get_answer_type() should not return None")
            
            # Test get_supported_methods
            methods = gen_instance.get_supported_methods()
            self.assertIsInstance(methods, list, 
                                 f"{gen_class.__name__}.get_supported_methods() should return a list")
            self.assertTrue(len(methods) > 0, 
                           f"{gen_class.__name__}.get_supported_methods() should return a non-empty list")
            for method in methods:
                self.assertIsInstance(method, str, 
                                     f"Methods in {gen_class.__name__}.get_supported_methods() should be strings")
                self.assertIn(method, ['frame', 'object', 'scene'])
