...
class COCOMetric(BaseMetric[Object2DAnswer]):
    ...
    def __init__(self, dataset_path: Path, iou_threshold: float = 0.5, top_k_values: List[int] = [1, 3, 5]):
    ...
    def reset(self):
        ...
        self.coco_gt = {"images": [], "annotations": [], "categories": [{"id": 1, "name": "object"}]}
        self.coco_dt = []
        self.img_id = 0
        self.ann_id = 0
        self.pred_id = 0
    ...
    def summarise(self, metric_results: List[Dict]) -> Dict:
        ...
        for prompt in self.prompts:
            ...
            prompt_dt_list = [det for det in self.coco_dt if det.get("prompt") == prompt]
    ...
    def _calculate_top_k_metrics(self, image_results: List[Dict]) -> Dict:
        ...
        prompt_topk = {prompt: {f"top{k}": 0 for k in self.top_k_values} for prompt in self.prompts}
    ...
    def evaluate(self, prediction: Object2DAnswer, ground_truth: Object2DAnswer, question: SingleImageQuestion) -> Dict[str, float]:
        ...
        if not isinstance(prediction, MultiObject2DAnswer) and not isinstance(ground_truth, MultiObject2DAnswer):
            # Add to COCO format
            img_id = self.img_id
            self.img_id += 1
            ...
            torch_iou = box_iou(
                torch.tensor(gt_box).reshape(1, 4), 
                torch.tensor(pred_box).reshape(1, 4)
            ).item()
            ...
            result = {
                ...
                "matches": [match],
                "prompt": prediction.prompt if hasattr(prediction, 'prompt') else "__noprompt__"
            }
        ...
        pred_multi = prediction if isinstance(prediction, MultiObject2DAnswer) else MultiObject2DAnswer.from_single_object(prediction)
        gt_multi = ground_truth if isinstance(ground_truth, MultiObject2DAnswer) else MultiObject2DAnswer.from_single_object(ground_truth)
    ...
    def vis_multi(self, predictions: MultiObject2DAnswer, ground_truths: MultiObject2DAnswer, question: SingleImageQuestion) -> None:
        ...
        try:
            ...
            # Add prompt to image
            prompt = predictions.prompt if hasattr(predictions, 'prompt') else "No prompt"
            prompt_text = f"Q: {prompt}"
            cv2.putText(
                img_vis, prompt_text, (10, 30), 
...
class COCOEvaluator:
    ...
    def __init__(self, dataset_path: Path, iou_thresholds=None, top_k_values=None):
        ...
        self.metrics = {
            iou: COCOMetric(dataset_path, iou_threshold=iou, top_k_values=self.top_k_values) 
    ...
    def evaluate(self, predictions, ground_truths, questions):
        ...
        for iou_threshold, metric in self.metrics.items():
            ...
            for i, (pred, gt, question) in enumerate(zip(predictions, ground_truths, questions)):
                try:
                    ...
                    metric_results.append({
                        "scores": [],
                        "matches": [],
                        "prompt": question.question if hasattr(question, 'question') else "__noprompt__"
                    })
        ...
        topk_metrics = {}
        for k in self.top_k_values:
            topk_values = [results[f"mAP@{iou:.2f}"].get(f"AP@top{k}", 0.0) for iou in self.iou_thresholds]
        ...
        per_prompt_topk = {prompt: {f"top{k}": [] for k in self.top_k_values} for prompt in prompt_set}
                    ...
                    if prompt in summary["per_prompt_topk"]:
                        for k in self.top_k_values:
                            ...
                            if key in summary["per_prompt_topk"][prompt]:
                                per_prompt_topk[prompt][key].append(summary["per_prompt_topk"][prompt][key])
        ...
        for prompt in prompt_set:
            ...
            for k in self.top_k_values:
                ...
                results["per_prompt_topk"][prompt][key] = sum(values) / len(values) if values else 0.0
