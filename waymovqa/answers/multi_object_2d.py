...
class MultiObject2DAnswer(BaseAnswer):
    ...
    def __init__(self, boxes: List[List[float]], scores: List[float] = None, prompt: str = None):
        self.boxes = boxes
        self.scores = scores if scores is not None else [1.0] * len(boxes)
        self.prompt = prompt
    ...
    def to_json(self):
        return json.dumps(
            dict(boxes=self.boxes, scores=self.scores, 
                 prompt=self.prompt, answer_type=str(self.answer_type))
        )